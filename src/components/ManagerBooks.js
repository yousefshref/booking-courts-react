...
const ManagerBooks = () => {
  ...
  const getBooks = async () => {
    setLoading(true);
    try {
      const res = await apiContext?.getBooks(
        dateFrom,
        dateTo,
        court,
        isCancelled,
      ...
      <div className="flex flex-col gap-4 p-5">
        <div className="search flex flex-col gap-2 bg-white p-4 rounded-md">
          <div className="flex flex-col sm:flex-row gap-5">
            ...
            <div className="flex flex-col gap-2 sm:w-1/3">
              <div className="w-full">
                ...
                >
                  <Select.Option value={""}>الجميع</Select.Option>
                  <Select.Option value={"True"}>نعم</Select.Option>
                  <Select.Option value={"False"}>لا</Select.Option>
                </Select>
              </div>
              <div className="w-full">
                <div className="flex flex-col gap-2 w-full">
                  ...
                  >
                    ...
                    <Select.Option value={"عند الحضور"}>
                      عند الحضور
                    </Select.Option>
                  </Select>
                </div>
          ...
          <Button
            onClick={() => getBooks()}
            type="primary"
            className="h-[40px] rounded-full w-[120px]"
          >
            بحث
          </Button>
        </div>
        ...
        <div className="flex flex-wrap gap-4 justify-around">
          {books?.length > 0 ? (
            ...
            <Alert message="لا يوجد حجوزات" type="warning" className="w-full" />
          )}
        </div>
