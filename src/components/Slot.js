...
const Slot = ({
  ...
  const pinned = pinnedSlots?.find(
    (p) =>
      Date("2000-01-01T" + slot?.split("-")[0]) ===
        Date(
          "2000-01-01T" + pinnedSlots[0]?.book_details?.start_time?.slice(0, 5)
        ) &&
      Date("2000-01-01T" + slot?.split("-")[1]) ===
        Date(
          "2000-01-01T" + pinnedSlots[0]?.book_details?.end_time?.slice(0, 5)
        ) &&
      Date(date) === Date(p?.book_details?.date)
  );

  const booked = bookedSlots?.find(
    (b) =>
      b?.start_time?.slice(0, 5) === first_slot?.slice(0, 5) &&
      b?.end_time?.slice(0, 5) === sec_slot?.slice(0, 5) &&
      b?.date === date
  );
  const b =
    (pinnedSlots[0]?.book_details?.start_time?.slice(0, 5) === first_slot &&
      pinnedSlots[0]?.book_details?.end_time?.slice(0, 5) === secSlot) ||
    (pinned?.book_details?.end_time?.slice(0, 5) === sec_slot &&
      pinned?.book_details?.start_time?.slice(0, 5) === first_slot) ||
    booked;
  ...
  return (
    hideSlotCuzt && (
      <div
        ...
        ${closed ? "bg-yellow-300 hover:bg-yellow-200" : b ? "bg-red-300 hover:bg-red-200" : "bg-white hover:bg-indigo-100"}
      `}
      >
