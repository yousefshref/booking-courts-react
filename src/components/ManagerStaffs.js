...
const ManagerStaffs = () => {
  ...
  const createStaffVerification = async () => {
    const signUpVerRes = await apiContext?.signUpSendVerification(
      username,
      phone,
      email,
      password
  ...
  const signUp = async () => {
    const res = await apiContext?.signUp(
      username,
      phone,
      email,
      password,
  ...
  const updateStaffUser = async (
    ...
    const res = await apiContext?.updateStaffUser(
      staff_id,
      username,
      phone,
      email,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Modal
        ...
        onOk={() => {
          verificationField ? signUp() : createStaffVerification();
        }}
        onCancel={() => setCourtStaffOpen(false)}
        className="font"
      >
        <div className="text-center flex flex-col gap-4">
          {alert === "error" && <Alert type="error" message="البيانات خاطئة" />}
          <h3 className="font-bold text-3xl">
            تسجيل الدخول بحسابك الموجود بالفعل
          </h3>
          <hr />
          <form
            onSubmit={(e) => {
          ...
          >
            ...
            <div
              className={`${verificationField ? "h-fit" : "h-0"} overflow-hidden transition-all flex flex-col gap-1 text-start`}
            >
      ...
      <Modal
        ...
        onOk={() => {
          verificationField
            ? updateStaffUser(
                staff?.user_details?.id,
                staffUsername,
                staffPhone,
                staffEmail,
                verification
              )
            : serndVerification(staffPhone);
        }}
      ...
      <div className="flex flex-col gap-4 p-5">
        ...
        <div className="flex gap-3">
          <Button
            onClick={() => setCourtStaffOpen(true)}
        ...
        <div className="flex flex-wrap gap-7">
          {staffs?.map((staff) => (
            <div
              key={staff?.id}
              className="p-3 shadow-md w-full sm:max-w-[300px] bg-white rounded-md flex flex-col gap-3"
            >
              ...
              <div className="actions flex gap-3 flex-col sm:flex-row">
                <Button
                  onClick={() => setOpenStaff(staff?.id)}
                  type="primary"
                  className="w-full mx-auto rounded-full sm:max-w-[200px]"
                  variant="contained"
                >
                ...
                >
                  <Button
                    type="default"
                    className="w-full rounded-full sm:max-w-[200px] mx-auto"
                    variant="outlined"
                  >
