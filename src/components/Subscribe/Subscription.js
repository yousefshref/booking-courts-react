...
const Subscription = ({ subscripe }) => {
  ...
  return (
    <div className='relative w-full gap-1 border border-zinc-700 rounded-xl p-3 bg-white'>
      <span onClick={() => setUpdateSubscripe(true)} className='text-blue-600 cursor-pointer absolute left-1 top-2 z-50'>
        <BiEdit />
      </span>
      {
        !profile?.user &&
        <span onClick={() => apiContext?.deleteSubscribe(subscripe?.id)} className='text-red-600 cursor-pointer absolute top-8 left-1 z-50'>
          <BiTrash />
        </span>
      }
      <div className='relative w-full flex flex-row overflow-x-scroll gap-7 justify-around'>
        ...
        <div className='flex flex-row gap-3 min-w-fit my-auto'>
          <p className='text-green-800 w-full my-auto'>{subscripe?.price} EGP</p>
          {
            profile?.user && !subscripe?.is_approved ? (
              <p className='text-red-800 my-auto min-w-fit max-w-[200px]'>لم يتم الموافقة علي طلب الاشتراك</p>
            ) :
              <>
                {
                  subscripe?.is_approved ? (
                    <>
                      <p className='text-zinc-700 my-auto min-w-fit max-w-[200px]'>يبدأ في: {subscripe?.start_from}</p>
                      <p className='text-zinc-700 my-auto min-w-fit max-w-[200px]'>ينتهي في: {subscripe?.end_to}</p>
                    </>
                  ) :
                    <>
                      <p className='text-red-700 my-auto min-w-fit max-w-[200px]'>طلب من لاعب علي الاشتراك</p>
                    </>
                }
              </>
          }
        </div>
        <small className='flex min-w-fit my-auto'>تم الانشاء في: {subscripe?.created_at}</small>
      </div>
    </div>
  )
}
