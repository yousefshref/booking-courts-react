...
const UpdateBook = ({
  ...
  return (
    ...
    >
      {contextHolder}
      <div className="flex flex-col gap-3 overflow-y-scroll h-full max-h-[440px]">
        <div id="1" className="1 flex flex-col gap-4">
          <div className="flex flex-col gap-1 p-3 rounded-md border">
            ...
            <DatePicker
              value={pinnedTo ? dayjs(pinnedTo, "YYYY-MM-DD") : ""}
              onChange={(e) => setPinnedTo(!e ? "" : e.format("YYYY-MM-DD"))}
              placeholder="أختر تاريخ التثبيت"
            />
            <small className="text-red-500">
              سوف يتم حجز نفس الفترة كل 7 ايام من تاريخ الحجز
            </small>
            {book?.pinned_to && (
              <Button
                onClick={() => setPinnedToDisplay(true)}
                type="link"
                className="w-fit"
              >
            ...
            <PinnedTimes
              getBooks={getBooks}
              book={book}
              open={pinnedToDisplay}
              setOpen={setPinnedToDisplay}
            />
          </div>
            ...
            <div className="flex flex-col gap-1 p-3 rounded-md border">
              <div className="text-zinc-600 flex gap-1">
                ...
                <small className="text-red-400">
                  سيتم اضافة {court?.ball_price} EGP
                </small>
              </div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e)
                    info(`تم اضافة سعر الكرة ${parseFloat(court?.ball_price)}`);
                }}
          ...
          {isEventTime() && (
            <div className="flex flex-col gap-1 p-3 rounded-md border">
              <div className="text-zinc-600 flex gap-1">
                ...
                <small className="text-red-400">
                  سيتم اضافة {parseFloat(court?.event_price)} EGP
                </small>
              </div>
              <select
                value={eventTime}
              ...
              <div
                onClick={() => {
                  ...
                  if (exist) {
                    ...
                    info(
                      `تم اضافة ${tool?.name} بسعر ${parseFloat(tool?.price)} EGP`
                    );
                  }
                }}
                key={tool?.id}
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <select
              ...
              className="p-2 rounded-md outline-none border transition-all hover:border-blue-500 focus:border-blue-600"
            >
              <option className="nodrag" value={"عند الحضور"}>
                الدفع في الملعب
              </option>
            </select>
          </div>
          ...
          {user?.user ? null : (
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                className="p-2 rounded-md outline-none border transition-all hover:border-blue-500 focus:border-blue-600"
              >
                <option className="nodrag" value={true}>
                  نعم
                </option>
                <option className="nodrag" value={false}>
                  لا
                </option>
              </select>
            </div>
