...
const CreateOrUpdateTrainers = ({ open, setOpen, trainer }) => {
  ...
  const [pricePerClass, setPricePerClass] = useState(trainer?.price_per_class || 0)
  const [pricePerWeek, setPricePerWeek] = useState(trainer?.price_per_week || 0)
  const [pricePerMonth, setPricePerMonth] = useState(trainer?.price_per_month || 0)
  const [pricePerYear, setPricePerYear] = useState(trainer?.price_per_year || 0)
    ...
    >
      ...
      <div className='flex flex-col gap-5 overflow-y-scroll min-h-fit max-h-[600px]'>
        <div className='flex flex-col gap-1'>
          ...
          />
          {
            typeof image === 'object' && image && image !== '' ? <img src={URL.createObjectURL(image)} alt="" /> : null
          }
          {
            typeof image === 'string' && image && image !== '' ? <img src={server + image} alt="" /> : null
          }
          {
            image &&
            <span onClick={() => setImage('')} className='text-red-600 cursor-pointer'>
              <FiDelete />
            </span>
          }
        </div>
