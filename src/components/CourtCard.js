...
const CourtCard = ({ court, deleteCourt }) => {
  ...
  useEffect(() => {
    if (court?.id) {
      ...
      setImages(court?.images_details);
      setVideos(court?.video_details);
      setTools(court?.tools_details);
      setFeatures(court?.features_details);
    }
  }, [court]);
  ...
  const updateCourt = async () => {
    try {
      ...
      const emptyFeatures = selectedFeatures?.find(
        (f) => f.is_free === null || f.is_free === undefined || f.name === ""
      );
      const emptyTools = selectedTools?.find(
        (t) =>
          t.name === "" ||
          t.price === null ||
          t.price === undefined ||
      ...
      const courtRes = await apiContext?.updateCourt(
        courtDetail?.id,
        name,
        address,
        locationUrl,
      ...
      if (courtRes?.data?.id) {
        const newcloseTime = selectedClosedTimes?.filter((time) => !time.id);
        const createCourtCloseTime = await apiContext?.createCourtCloseTime({
          data: newcloseTime,
          court_id: courtRes?.data?.id,
        });

        const existCloseTime = selectedClosedTimes?.filter((time) => time.id);
        const updateCourtCloseTime = await apiContext?.updateCourtCloseTime({
          data: existCloseTime,
        });

        const createImagesRes = await apiContext?.createCourtImage(
          courtRes?.data.id,
          createImages
        );
        const createCourtVideoRes = await apiContext?.createCourtVideo(
          courtRes?.data.id,
          createVideos
        );
        ...
        const updateCourtFeature =
          await apiContext?.updateCourtFeature(updateFeatures);
    ...
    <div className="courtCard h-fit p-3 bg-white w-full sm:max-w-[430px] flex flex-col gap-4">
      ...
      <div className="flex justify-between gap-3">
        ...
        <div className="flex flex-col my-auto">
          <p>
            من: {convertToAMPM(court.open_to)} حتي:{" "}
            {convertToAMPM(court.open_from)}
          </p>
          {court.close_from && court.close_to ? (
            <p className="text-xs text-zinc-700">
              يغلق بين: {convertToAMPM(court.close_from)} و{" "}
              {convertToAMPM(court.close_to)}
            </p>
          ) : null}
        </div>
      </div>
      ...
      <div className="flex justify-between gap-3 px-2">
        <div className="flex flex-col text-center gap-1">
          <p className="text-2xl">
            {convertToK(String(courtDetail?.total_books)) ?? 0}{" "}
            <span className="text-sm">مرة</span>
          </p>
          <p className="text-xs text-zinc-700">تم الحجز</p>
        </div>
        <div className="flex flex-col text-center gap-1">
          <p className="text-2xl">
            {convertToK(String(courtDetail?.total_money)) ?? 0}{" "}
            <span className="text-sm">جنية</span>
          </p>
        ...
        <div className="flex gap-1">
          ...
          <div className="flex flex-col my-auto">
            <small
              onClick={() =>
                navigate(
                  `/court/${court?.name?.replace(" ", "-")}/${court?.id}`
                )
              }
              className="text-xs cursor-pointer text-blue-600 underline underline-offset-4"
            >
              {court?.name}
            </small>
          </div>
