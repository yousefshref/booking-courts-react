      ...
      <div className="flex flex-col">
        ...
        {item?.price_per_class !== 0 && (
          <p className="text-green-600">
            {item?.price_per_class} EGP / في الحصة
          </p>
        )}
        {item?.price_per_week !== 0 && (
          <p className="text-green-600">
            {item?.price_per_week} EGP / في الاسبوع
          </p>
        )}
        {item?.price_per_month !== 0 && (
          <p className="text-green-600">
            {item?.price_per_month} EGP / في الشهر
          </p>
        )}
        {item?.price_per_year !== 0 && (
          <p className="text-green-600">
            {item?.price_per_year} EGP / في السنة
          </p>
        )}
      </div>
      <Button
        onClick={() => setOpenSubscribe(true)}
        className="font h-[48px] bg-indigo-500 w-full"
        type="primary"
      >
        اشتراك
      </Button>
      ...
      {!apiContext?.profile?.user && (
        <>
          <Button
            ...
            className="font h-[48px] bg-blue-500 w-full"
            type="primary"
          >
            تعديل
          </Button>
          <Button
            onClick={() => apiContext?.deleteTrainer(item?.id, setOpen)}
            className="font h-[48px] bg-red-500 w-full"
            type="primary"
          >
            حذف
          </Button>
