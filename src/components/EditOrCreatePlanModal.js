...
const EditOrCreatePlanModal = ({
  ...
  useEffect(() => {
    if (!create && plan?.id) {
      ...
      plan?.trainer && setTrainer(plan?.trainer);
  ...
  return (
    ...
    >
      ...
      <div className="p-4 relative h-full max-h-[450px] overflow-y-scroll bg-indigo-200 flex rounded-xl flex-col gap-3">
        <div className="bg-white p-3 rounded-xl flex flex-col gap-1">
          <p>اسم الخطة *</p>
          <Input
            placeholder="مثال: خطة الاطفال / الخطة السنوية"
            value={nameState}
        ...
        <div className="bg-white p-3 rounded-xl flex flex-col gap-1">
          <p>المدرب</p>
          <Select value={trainer} onChange={(e) => setTrainer(e)}>
            <Select.Option value="">أختر</Select.Option>
            {apiContext?.trainers?.map((trainer) => (
        ...
        <div className="bg-white p-3 rounded-xl flex flex-col gap-1">
          ...
          <Input
            type="number"
            value={pricePerClassState}
            onChange={(e) => setPricePerClassState(e.target.value)}
            placeholder="السعر المدفوع من كل حصة"
          />
        </div>
        <div className="bg-white p-3 rounded-xl flex flex-col gap-1">
          ...
          <Input
            type="number"
            value={pricePerWeekState}
            onChange={(e) => setPricePerWeekState(e.target.value)}
            placeholder="السعر المدفوع في الاسبوع"
          />
        </div>
        <div className="bg-white p-3 rounded-xl flex flex-col gap-1">
          ...
          <Input
            type="number"
            value={pricePerMonthState}
            onChange={(e) => setPricePerMonthState(e.target.value)}
            placeholder="السعر المدفوع في الشهر"
          />
        </div>
        <div className="bg-white p-3 rounded-xl flex flex-col gap-1">
          ...
          <Input
            type="number"
            value={pricePerYearState}
            onChange={(e) => setPricePerYearState(e.target.value)}
            placeholder="السعر المدفوع في السنة"
          />
        </div>
