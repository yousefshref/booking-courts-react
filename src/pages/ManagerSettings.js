      ...
      <div className="flex flex-col gap-4 p-4 w-full max-w-4xl mx-auto">
        <div className="flex gap-2 justify-between flex-wrap">
          ...
          <small className="text-blue-600">
            يمكنك اضافة اشخاص يمكنهم الحجز بدون قيود, او الدفع في اي وقت
          </small>
        </div>
        <hr />
        <div className="flex flex-col gap-4 bg-white p-4 shadow-md">
          <div className="flex flex-col gap-2">
            ...
            <Input
              className="p-4"
              value={whitelist?.phone}
              onChange={(e) =>
                setWhitelist({ ...whitelist, phone: e.target.value })
              }
            />
          </div>
          <div className="flex flex-col gap-2">
            ...
            <Input
              ...
              className="p-4"
              onChange={(e) =>
                setWhitelist({ ...whitelist, image: e?.target?.files[0] })
              }
            />
          </div>
          <div className="flex flex-col gap-2">
            ...
            <Input
              ...
              className="p-4"
              onChange={(e) =>
                setWhitelist({ ...whitelist, image2: e?.target?.files[0] })
              }
            />
          </div>
          <div className="flex gap-5 flex-wrap">
            <Button
              onClick={createWhtieList}
              className="font w-fit rounded-xl bg-green-500"
              type="primary"
            >
              انشاء
            </Button>
            <Button
              onClick={() => setDisplayWhiteLists(true)}
              className="font w-fit rounded-xl"
              type="default"
            >
              رؤية القائمة البيضاء
            </Button>
          </div>

          {/* whitelists */}
          <DisplayWhiteLists
            open={displayWhiteLists}
            setOpen={setDisplayWhiteLists}
          />
        </div>
      </div>
    </Modal>
